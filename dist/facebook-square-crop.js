//https://raw.github.com/tripdin/facebook-square-crop/master/dist/facebook-square-crop.js
...
(function() {
  ...
  FacebookSquareCrop = (function() {
    ...
    FacebookSquareCrop.prototype.crop = function(data) {
      //console.log('crop, '+data.length);
      var crop, item, set, style, _i, _len, _results;
      _results = [];
      for (_i = 0, _len = data.length; _i < _len; _i++) {
        item = data[_i];
        crop = item.pic_crop;
   if($(".fbImage" + item.id).is('img')){
    // log('is image!');
    ...
    }else{ //If it's a DIV tag, use BG image standard...
      //log('is a DIV !');
        if (crop.width > crop.height) {
          set = Math.min((crop.width / crop.height) * (crop.left / crop.bottom), 1 - (crop.height / (crop.width - 1)));
          style = {
